...
export function DrivePage() {
  ...
  const { flyTo, drawRoute, clearRoute } = useMap();
  ...
  useEffect(() => {
    ...
    if (routePointsRef.current.length >= 2) {
      drawRoute(routePointsRef.current);
    }

    prevSpeedRef.current = speedKmh;
    prevPositionRef.current = position;
  }, [isRecording, position, flyTo, drawRoute, haversineDistance]);
  ...
  }, [navigate, flyTo, clearRoute, stopTracking, addTrip, user, calculateFuelCost]);
  ...
  return (
    <div className="fixed inset-0">
      ...
      <AnimatePresence>
        {showRouteSearch && (
          <RouteSearch isOpen={showRouteSearch} onClose={() => setShowRouteSearch(false)} />
        )}
      </AnimatePresence>
    </div>
  );
